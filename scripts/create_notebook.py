...
add_code("""# Cell 2: Loading the telemetry dataset (Colab & local compatible)
# Public raw GitHub URL for the dataset (works directly in Google Colab & Kaggle)
...
add_code("""# Cell 3: Data preprocessing and cleaning pipeline
...
# 2. Check and remove duplicate timestamps
duplicate_count = df_clean.duplicated(subset=['timestamp']).sum()
if duplicate_count > 0:
    print(f"- Found {duplicate_count} duplicate records. Dropping duplicates...")
...
if total_nulls > 0:
    print("- Applying forward-fill followed by backward-fill...")
    df_clean = df_clean.ffill().bfill()
else:
    print("- Clean dataset: No missing values.")

# 4. Physical boundary sanity checks and range clamping
...
# Latency, discards, and throughput must be strictly non-negative
